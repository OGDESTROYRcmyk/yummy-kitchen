...
$("#orderButton").onclick=()=>{update();open("cartModal")};$("#checkoutButton").onclick=()=>{if(!cart.length){toast("Add some food first! 🍔");return}close("cartModal");open("checkoutModal")};$("#placeOrderButton").onclick=()=>{const total=cart.reduce((a,x)=>a+x.price,0);$("#receiptContent").innerHTML=cart.map(x=>`<div class="receipt-line"><span>${x.name}</span><strong>${money(x.price)}</strong></div>`).join("")+`<div class="receipt-total"><span>Total</span><span>${money(total)}</span></div><p>🕒 Fictional kitchen time: 18–27 minutes.</p>`;cart.length=0;update();close("checkoutModal");open("receiptModal")};
...
function playPromo(){fakeVideo.classList.add('playing');promoPlaying=true;promoProgress.classList.add('running');promoPlay.textContent='↻ Replay Promo';promoPause.textContent='Ⅱ'}
promoPlay?.addEventListener('click',playPromo);
promoPause?.addEventListener('click',()=>{promoPlaying=!promoPlaying;fakeVideo.classList.toggle('playing',promoPlaying);promoProgress.classList.toggle('running',promoPlaying);promoPause.textContent=promoPlaying?'Ⅱ':'▶'});
promoBack?.addEventListener('click',()=>{fakeVideo.classList.remove('playing');promoPlaying=false;promoProgress.classList.remove('running');promoPause.textContent='▶';promoPlay.textContent='▶ Play Promo'});
